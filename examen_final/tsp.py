...
class TravelingSalesmanProblem:
  ...
  def __createData(self):
    """Reads the desired TSP file from the Internet, extracts the city
    coordinates, calculates the distances
    between every two cities and uses them to populate a distance matrix (twodimensional array).
    It then serializes the city locations and the calculated distances to disk
    using the pickle utility.
    """
    self.locations = []
  ...
  def plotData(self, indices):
    ...
    plt.title("Optimal Solution", size=8)
    plt.xlabel("X")
    plt.ylabel("Y")
