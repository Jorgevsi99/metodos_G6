...
%Funcion regresion lineal: la cual permite el ingreso de dos vectores los
...
%REGRESION_LINEAL
  %REGRESION_LINEAL(X,Y)
    ...
    %sr = residuo entre la Y medida y la Y calculada por el metodo
    %recta=ecuacion de la recta evaluada con los coeficientes en C

function [sr,recta ] = regresion_lineal( X,Y )
  ...
  if nargin == 2
    ...
    if n==n1
        ...
        %sumatoria (ecuacion 17.3)
        for i=1:n
            sr=sr+(Y(i)-a0-a1*X(i))^2;%calculo de la suma de los residuos cuadrados
        end
